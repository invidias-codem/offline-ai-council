...
## Technology Stack
  ...
  * **Backend:** **Rust** (with [Axum](https://github.com/tokio-rs/axum) & [Tokio](https://tokio.rs/)) for its high performance and near-zero memory footprint.
...
You must install all of the following:
  ...
  * **Ollama (Manual Install):**
      ...
      * Go to the [Ollama GitHub Releases](https://github.com/ollama/ollama/releases).
...
```bash
git clone https://github.com/invidias-codem/offline-ai-council.git
...
1.  Open the `server_manager/config.json` file.

2.  Update the `rust_backend_path` and `pwa_folder_path` to their **full, absolute paths** on your machine.
...
1.  **Start Docker:** Open the Docker Desktop app.
2.  **Start ChromaDB:** In a terminal, start the vector database:
    ```bash
    docker run -d -p 8000:8000 --name chromadb chromadb/chroma
    ```
3.  **Add Documents:** Place your `.pdf`, `.txt`, `.md`, or code files into the `ai_ingestion/my_documents/` folder.
...
1.  `cd` into the `server_manager` folder.
2.  Run the **start** command:
    ```bash
    python3 manager.py start
    ```
3.  This will open 6 new terminal windows, one for each service:
      * Ollama Server
      * ChromaDB (Docker)
      * Rust Backend
      * PWA Frontend Server
...
1.  On your Mac, find your Local IP (System Preferences \> Network \> Wi-Fi). It will be like `192.168.1.XX`.
2.  On your phone (on the *same* Wi-Fi), open a browser and go to `http://<YOUR_LAPTOP_IP>:3000`.
3.  You must edit `pwa_frontend/app.js` to point to this IP. Find the `fetch` call (around line 55) and change it from `localhost:8080` to `<YOUR_LAPTOP_IP>:8080`.

#### External Access (From Anywhere)

This is the recommended setup.

1.  Look at your `ngrok http 3000` terminal. It will give you a public URL:
      * `https://random-string-1.ngrok-free.app`
2.  Look at your `ngrok http 8080` terminal. It will give you a *different* public URL:
      * `https://random-string-2.ngrok-free.app`
3.  Edit `pwa_frontend/app.js` one time. Change line 55 to use your Rust backend's ngrok URL:
    ```javascript
    // BEFORE
    // const response = await fetch("http://localhost:8080/api/council", {

    // AFTER
    const response = await fetch("https://random-string-2.ngrok-free.app/api/council", {
    ```
4.  Save the file. Your Python server will auto-reload.
5.  On your phone, open your browser and go to the **frontend URL** (`https://random-string-1.ngrok-free.app`). You can now access your home AI from anywhere in the world.
